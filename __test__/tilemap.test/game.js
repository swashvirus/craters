...
class ball extends cg.Sprite {
  constructor (scope) {
    super(scope, {frames: [12], image: media.fetch('./tilemap.png')})
	this.scope = scope;
	...
	this.state.vel = {x: ((Math.random() - 0.5) * 5), y: ((Math.random() - 0.5) * 5)} // initiatal velocity
	this.state.size = { x: 32, y: 32 } // dimensions
  ...
  update () {
	  ...
	  this.state.pos.y = cg.boundary((this.state.pos.y), (this.state.size.y), (this.scope.state.size.y - (this.state.size.y)))
  }
	...
	constructor (scope, data){
		...
		class cell extends cg.Sprite {
			constructor (scope, args) {
				super(scope, {frames: args.frames})
		...
		this.map.m.forEach((tile) => {
			...
			this.scope.entities.push(new cell(this.scope , {image: this.image, pos: {x: tile[0] * this.map.w , y: tile[1] * this.map.h}, frames: [tile[2]], size: {x: this.map.w, y: this.map.h }}));
		})
	}
}
