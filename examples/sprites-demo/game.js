'use strict';
import { Game, Loop, Canvas, Entity, Sprite, Loader } from '../../app/craters/craters.js'
...
class mygame extends Game {
	constructor (container, width, height){
		...
		for (var i = 0; i < 15; i++) {
			this.entities.push(new boltbug(this, { pos: { x: (Math.random() * this.state.size.x), y: (Math.random() * this.state.size.y) } }))
		}
...
class ladybug extends Sprite {
	constructor (scope) {
		super(scope, {frames: [0, 1, 2]})
		this.scope = scope;
		this.state.image = media.fetch('./media/bug.png')
		this.state.size = { x: 196, y: 218 }
		this.state.pos = { x: (scope.state.size.x / 2) - (this.state.size.x / 2), y: (scope.state.size.y - this.state.size.y) }
...
media.load([
	'./media/bug.png',
	'./media/bolt.png'
], function () { window.game = new mygame('#container', window.innerWidth, window.innerHeight, 60, true)})